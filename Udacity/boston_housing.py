...
def explore_city_data(city_data):
    ...
    print "Number of houses in total : ", housing_features.shape[0]
    # Number of features?
    ...
    print "Standard deviation : ", round(np.std(city_data.target), 4)
...
def performance_metric(label, prediction):
    ...
    RMSE = mean_squared_error(label, prediction)**0.5
    return RMSE
...
def fit_predict_model(city_data):
    ...
    scores = make_scorer(performance_metric, 
                                 greater_is_better=False)

    reg = GridSearchCV(regressor, parameters, scoring=scores, verbose=True)
