...
class SupportVectorMachine:
         ...
         def loadDataSet(self,trainfile):
			     ...
			     if float(items[1])>1 or float(items[6])!=0.0:
	 ...
	 def test(self):
            ...
            for depth_num in range(1,10):
                 for num in range(1,100):
                       #params = {'n_estimators': 1200, 'max_depth': depth_num, 'subsample': 0.5,'learning_rate': 0.01, 'min_samples_leaf': min_leaf, 'random_state': 3}
                       #clf = GradientBoostingClassifier(**params)
                       clf=AdaBoostClassifier(DecisionTreeClassifier(max_depth=depth_num),algorithm="SAMME.R",n_estimators=num*10)
                       ...
                       if scores.mean()>0.9 and scores.std()<0.02:
                           print "********************************************************"
                           print "max_depth =", depth_num, "min_samples_leaf", num*100
