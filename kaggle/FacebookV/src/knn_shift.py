...
def process_grid(train, test, x_cuts, y_cuts, t_cuts, x_border_aug, y_border_aug, time_aug, fw, th, n_neighbors):
    preds_list = []
    ...
    for i in range(x_cuts):
        ...
        #x_min = x_slice * i
        #x_max = x_slice * (i+1)
        shift_x = x_slice* x_shift_ratio 
        x_min = x_slice * i + shift_x
        if x_min > x_boundary[1] or x_min < x_boundary[0] :
            x_min = x_slice * i
        x_max = x_slice * (i+1) + shift_x
        if x_max > x_boundary[1] or x_max < x_boundary[0] :
            x_max = x_slice * (i+1)
        ...
        for j in range(y_cuts):
            shift_y = y_slice* y_shift_ratio 
            y_min = y_slice * j + shift_y
            if y_min > y_boundary[1] or y_min < y_boundary[0] :
                y_min = y_slice * j
    
            y_max = y_slice * (j+1) + shift_y
            if y_max > y_boundary[1] or y_max < y_boundary[0] :
                y_max = y_slice * (j+1)
            ...
            for t in range(t_cuts):
                ...
                cell_pred = process_one_cell(cell_train, cell_test,fw, th, n_neighbors)
