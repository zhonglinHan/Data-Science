                 ...
                 for depth in range(1,20):
                     for split in range(1,20):
                        for leaf in range(1,20):
                                clf_Dtree = DecisionTreeClassifier(max_depth=depth,min_samples_split=split,min_samples_leaf=leaf)
                                clf_Dtree.fit(X_train, y_train)
                                y_pred = clf_Dtree.predict(X_test);
                                y_predprob = clf_Dtree.predict_proba(X_test);
                                if roc_auc_score(y_test,y_predprob[:,1])>0.85:
                                   print "Decision Tree","depth=",depth, " split=",split," leaf=",leaf
